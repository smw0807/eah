...
백엔드에서 **1분마다** Cron으로 경매 상태를 자동 갱신합니다.

- **경매 시작 처리** (`handleScheduledAuctions`)
  - 상태가 `SCHEDULED`이고 **시작 시각(`startAt`)이 지난** 경매를 조회
  - 해당 경매 상태를 `OPEN`(진행 중)으로 변경
  - **WebSocket**으로 상태 변경을 브로드캐스트해 상세 페이지가 실시간 반영

- **경매 종료 처리** (`handleExpiredAuctions`)
  - 상태가 `OPEN`이고 **종료 시각(`endAt`)이 지난** 경매를 조회
  - 해당 경매 상태를 `CLOSED`로 변경하고, 최고가 입찰을 `winningBidId`로 저장
  - **금액 정산** (`settleAuction`):
    - **낙찰자가 있는 경우**: 낙찰자의 잠금 금액을 차감하고, 판매자 계좌에 낙찰 금액 입금
    - **입찰이 없는 경우**: 금액 정산 없이 종료만 처리
  - **WebSocket**으로 상태 변경을 브로드캐스트

이 스케줄러 덕분에 등록 시 설정한 시작/종료 시각에 맞춰 경매가 자동으로 **시작**되고 **종료**되며, 종료 시 낙찰 금액이 판매자에게 자동 정산됩니다.
...
## 경매 상품 목록
카테고리, 가격, 상태, 정렬, 검색 등으로 검색 필터를 할 수 있다.   
대기, 진행중, 종료, 취소 상태를 각 상품에 표기하고 있으며, 종료의 경우는 크게 표시한다.
<img src="imgs/1.png"/>

## 경매 상품 등록
경매 상품을 등록하는 화면으로 입찰 단위에 따라 입찰이 이루어지게 된다.
<img src='imgs/2.png'/>

## 자기가 등록한 경매 상품 상세 화면
자신이 등록한 경매 상품에는 입찰할 수 없고, 수정 또는 취소 할 수 있다.
<img src='imgs/3.png'/>

## 경매 상품 수정
아직 입찰 목록이 없을 경우에만 수정할 수 있다.
<img src='imgs/4.png'/>
...
## 경매 입찰 후 화면
경매 입찰 시 웹소켓을 통해 즉시 화면에 가격을 업데이트 한다.   
같은 화면을 보고 있을 사용자들에게 즉각 업데이트 된 정보를 보여주기 위해 웹소켓을 사용했다.
<img src='imgs/6.png'/>

## 즉시 구매
즉시 구매시 즉시 구매가로 입찰을 해서 경매를 끝낸다.   
해당 경매는 종료 처리가 된다.
<img src='imgs/7.png'/>

## 마이페이지
자신의 프로필 정보, 계좌 정보, 낙찰 경매, 등록 경매, 입찰 내역 정보를 볼 수 있다.
<img src='imgs/8.png'/>
<img src='imgs/9.png'/>

### 입찰한 경매 상품이 있을 경우.
입찰 한 경매 상품의 가격만금 잠금 잔액이 생긴다.   
해당 금액을 제외한 금액만을 사용할 수 있다.
<img src='imgs/10.png'/>